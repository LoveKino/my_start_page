const parseConsoleText = (text, sandbox, env) => {
  ...
  env.printLog(`> command ${commandName}`);
  if (!sandbox.hasOwnProperty(commandName)) {
    env.printErr(`Can't find command ${commandName}`);
  } else {
    ...
    try {
      ...
      env.printErr(`Fail to execute command ${commandName}, error message is '${err.message}'.`);
    }
  }
};
